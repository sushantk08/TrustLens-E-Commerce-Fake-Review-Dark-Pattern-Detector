...
def compute_true_trust_score(
    ...
    if not reviews or len(reviews) == 0:
        return {
            ...
            'aspects_sentiment': {},
            'summary_reasons': [
                'Insufficient review data to compute a reliable Trust Score.'
            ],
            'components': {}
        }
    ...
    credibility_res = analyze_reviewer_credibility(
        reviews,
        category=category
    )

    dark_patterns_res = analyze_dark_patterns(
        price_history or [],
        current_price,
        original_price
    )
    ...
    aspects_res = extract_aspect_sentiments(
        reviews,
        category=category
    )
    ...
    # Penalty A: Review Velocity & Rating Distribution
    # Up to 25 points
    ...
    all_reasons.extend(
        velocity_res.get('flagged_reasons', [])
    )

    # Penalty B: Reviewer Credibility
    # Up to 25 points
    cred_score = credibility_res.get(
        'credibility_score',
        100.0
    )
    ...
    all_reasons.extend(
        credibility_res.get('flagged_reasons', [])
    )

    # Penalty C: Semantic Duplicates / Spun Reviews
    # Up to 25 points
    spun_ratio = semantic_res.get(
    ...
    all_reasons.extend(
        semantic_res.get('flagged_reasons', [])
    )

    # Penalty D: Dark Patterns
    # Artificial Pricing / Fake Urgency
    # Up to 15 points
    if dark_patterns_res.get(
        'price_manipulation_detected'
    ):
        score -= 10.0

    if dark_patterns_res.get(
        'fake_scarcity_detected'
    ):
        score -= 5.0

    all_reasons.extend(
        dark_patterns_res.get('flagged_reasons', [])
    )

    # Penalty E: Aspect Deficits
    # Hidden Flaws - Up to 10 points
    if aspects_res.get('flagged_reasons'):
        score -= min(
            10.0,
            len(
                aspects_res['flagged_reasons']
            ) * 5.0
        )
    ...
    # Calculate estimated percentage of suspicious/
    # manipulated reviews
    total_revs = max(
        1,
        len(reviews)
